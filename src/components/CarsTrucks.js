...
function CarsTrucks() {
  ...
  async function createCarsTrucks() {
    ...
    if (guest) {
      ...
      if (!formData.make_model || !formData.build_year || !formData.color || !formData.type || !formData.condition || !formData.price || !formData.city || !validatePhoneNumber(formData.phone)) return;
      updateFormData();
