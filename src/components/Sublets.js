...
import { listSublets } from '../graphql/queries';
...
function Sublet() {
  ...
  async function createSublet() {
    ...
    if (guest) {
      ...
      if ( !formData.housing_type || !formData.private_room || !formData.private_bath || !formData.laundry || !formData.parking || !formData.handicap_accessible || !formData.animals || !formData.num_bedrooms || !formData.num_bathrooms || !formData.flooring || !formData.sqft || !formData.rent_period || !formData.rent || !formData.city || !validatePhoneNumber(formData.phone)) return;
      updateFormData();
