...
import { getVacationRentals, listVacationRentals } from '../graphql/queries';
...
function VacationRentals() {
  ...
  async function createVacationRental() {
    if ( !formData.housing_type || !formData.laundry || !formData.parking || !formData.animals || !formData.handicap_accessible || !formData.num_bedrooms || !formData.num_bathrooms || !formData.flooring || !formData.sqft || !formData.availability || !formData.rent_period || !formData.rent || !formData.city || !formData.phone) return;
    updateFormData();
